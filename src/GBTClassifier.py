...
def train_and_evaluate_gbt_classification(df: DataFrame,
                                          feature_cols: Optional[List[str]] = None,
                                          categorical_cols: Optional[List[str]] = None,
    ...
    ovr = OneVsRest(classifier=gbt, labelCol=label_col, featuresCol="features")

    print(f"\n--- Тренування GBT Classifier (OneVsRest) ---")
    fitted = ovr.fit(train)
    preds = fitted.transform(test)
    
    metrics = _compute_metrics_safe(preds, label_col)

    print("\n[TEST EVALUATION - GBT Classifier]")
    ...
    print("\n[Sample predictions]")
    select_cols = [label_col, "prediction"]
    
    if "probability" in preds.columns:
        select_cols.append("probability")

    print("\nRows with trip_category = Long (2.0):")
    preds.filter(preds[label_col] == 2.0).select(*select_cols).show(5, truncate=False)

    print("\nRows with trip_category = Medium (1.0):")
    preds.filter(preds[label_col] == 1.0).select(*select_cols).show(5, truncate=False)

    print("\nRows with trip_category = Short (0.0):")
    preds.filter(preds[label_col] == 0.0).select(*select_cols).show(5, truncate=False)

    train.unpersist()

    results = [("GBT_Multiclass", metrics["accuracy"], metrics["precision"], metrics["recall"], metrics["f1"])]
