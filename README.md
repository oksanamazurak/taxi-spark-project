...
## Моделювання: Порівняння та висновки

Нижче наведено порівняння моделей для задач регресії (прогноз `total_amount`) та класифікації (`trip_category`):

`trip_category`, сформована з `trip_distance` за трьома інтервалами:
- 0.0 (Short): `trip_distance < 2` миль
- 1.0 (Medium): `2 ≤ trip_distance < 10` миль
- 2.0 (Long): `trip_distance ≥ 10` миль

![Порівняння моделей](results.png)


### Регресія
- **GBTRegressor** найкращий: RMSE ≈ 2.30, R² ≈ 0.91.
- **RandomForest** гірший: RMSE ≈ 3.22, R² ≈ 0.83.
- **LinearRegression** значно поступається: RMSE ≈ 5.83, R² ≈ 0.43.
- **Висновок:** Градієнтний бустинг добре справляється з прогнозуванням суми поїздки. Random Forest показує теж непогані результати. Лінійна регресія має найгірший результат через обмеженість лінійної моделі.

### Класифікація
- **RandomForest** має найвищі метрики: Accuracy ≈ 0.892, Weighted F1 ≈ 0.889.
- **GBT** майже на рівні: Accuracy ≈ 0.891, Weighted F1 ≈ 0.888.
- **Logistic Regression** трохи відстає: Accuracy ≈ 0.881, Weighted F1 ≈ 0.876.
- **Висновок:** Ансамблеві моделі (RandomForest та GBT) краще обробляють мультикласові залежності у даних про поїздки. Логістична регресія працює гірше через обмежену здатність моделювати складні взаємозв’язки між ознаками.
